...
# ── Farben (hell) ──────────────────────────────────────────────────────────────
LIGHT = {
    "BG": "#F5F0F0", "PANEL": "#FFFFFF", "PRIMARY": "#C0392B",
    "PRIMARY_H": "#A93226", "DANGER": "#E74C3C", "TEXT": "#2C2C2C",
    "SUBTEXT": "#7F7F7F", "BORDER": "#E8DEDE", "ROW_ODD": "#FDF6F6",
    "ROW_EVEN": "#FFFFFF", "HDR_BG": "#C0392B",
}
DARK = {
    "BG": "#1E1E2E", "PANEL": "#2A2A3E", "PRIMARY": "#C0392B",
    "PRIMARY_H": "#A93226", "DANGER": "#E74C3C", "TEXT": "#E0E0E0",
    "SUBTEXT": "#888899", "BORDER": "#3A3A55", "ROW_ODD": "#252535",
    "ROW_EVEN": "#2A2A3E", "HDR_BG": "#C0392B",
}
C = dict(LIGHT)  # aktives Theme
...
DATA_FILE   = os.path.join(os.path.dirname(os.path.abspath(__file__)), "notizen_data.json")
TF_FILE     = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tf_data.json")
...
# ── Popup: Notiz hinzufügen/bearbeiten ────────────────────────────────────────
class NotePopup(tk.Toplevel):
    def __init__(self, parent, on_confirm, existing=None, stw_options=STW_OPTIONS, fahrt_options=FAHRT_OPTIONS, tf_mode=False):
        super().__init__(parent)
        self.on_confirm = on_confirm
        self.existing = existing
        self.tf_mode = tf_mode
        self.stw_options = stw_options
        self.fahrt_options = fahrt_options
        self.title("Notiz bearbeiten" if existing else "Neue Notiz")
        self.resizable(False, False)
        self.configure(bg=C["BG"])
        self.grab_set()
        w, h = (420, 560) if not tf_mode else (420, 480)
    ...
    def _build(self):
        ...
        hdr.pack(fill="x")
        title = "TF-Notiz bearbeiten" if (self.existing and self.tf_mode) else \
                "Neue TF-Notiz" if self.tf_mode else \
                "Notiz bearbeiten" if self.existing else "Neue Notiz"
        tk.Label(hdr, text=title, font=("Segoe UI", 12, "bold"),
                 bg=C["PRIMARY"], fg="white").pack(side="left", padx=16, pady=12)
        tk.Label(hdr, text="  X  ", font=("Segoe UI", 12, "bold"),
                 bg=C["PRIMARY"], fg="white", cursor="hand2").pack(side="right", padx=8)
        ...
        if not self.tf_mode:
            ...
            self.stw_var = tk.StringVar(value=(self.existing or {}).get("stw", self.stw_options[0]))
            row = tk.Frame(body, bg=C["BG"]); row.pack(anchor="w", pady=4)
            self._stw_btns = {}
            for s in self.stw_options:
                b = tk.Button(row, text=s, font=("Segoe UI", 9, "bold"),
                              bg=C["BORDER"], fg=C["TEXT"], relief="flat", cursor="hand2",
                              padx=12, pady=4, bd=0, command=lambda s=s: self._sel_stw(s))
            ...
            self.fahrt_var = tk.StringVar(value=(self.existing or {}).get("fahrt", self.fahrt_options[0]))
            fr = tk.Frame(body, bg=C["BG"]); fr.pack(anchor="w", pady=4)
            self._fahrt_btns = {}
            for s in self.fahrt_options:
                b = tk.Button(fr, text=s, font=("Segoe UI", 8, "bold"),
                              bg=C["BORDER"], fg=C["TEXT"], relief="flat", cursor="hand2",
                              padx=8, pady=3, bd=0, command=lambda s=s: self._sel_fahrt(s))
            ...
            self.gleis = tk.Entry(body, font=FONT_INPUT, relief="flat", bg=C["PANEL"],
                                  fg=C["TEXT"], insertbackground=C["TEXT"],
                                  highlightthickness=1, highlightbackground=C["BORDER"],
                                  highlightcolor=C["PRIMARY"])
            self.gleis.pack(fill="x", ipady=5, pady=4)
            if self.existing:
                self.gleis.insert(0, self.existing.get("gleis", ""))
            tk.Frame(body, bg=C["BORDER"], height=1).pack(fill="x", pady=8)

            # Zugnummern
            tk.Label(body, text="Zugnummern (z.B. 1234,5678)", font=FONT_LABEL, bg=C["BG"], fg=C["SUBTEXT"]).pack(anchor="w")
            self.zug = tk.Entry(body, font=FONT_INPUT, relief="flat", bg=C["PANEL"],
                                fg=C["TEXT"], insertbackground=C["TEXT"],
                                highlightthickness=1, highlightbackground=C["BORDER"],
                                highlightcolor=C["PRIMARY"])
            self.zug.pack(fill="x", ipady=5, pady=4)
            if self.existing:
                self.zug.insert(0, self.existing.get("zuege", ""))
            tk.Frame(body, bg=C["BORDER"], height=1).pack(fill="x", pady=8)

        # Prio & Status
        ps_row = tk.Frame(body, bg=C["BG"]); ps_row.pack(fill="x", pady=2)
        # Priorität
        pl = tk.Frame(ps_row, bg=C["BG"]); pl.pack(side="left", fill="x", expand=True)
        ...
        pr = ttk.Combobox(pl, textvariable=self.prio_var, values=PRIO_OPTIONS,
                          state="readonly", font=FONT_INPUT, width=10)
        pr.pack(anchor="w", pady=2)
        # Status
        sl = tk.Frame(ps_row, bg=C["BG"]); sl.pack(side="left", fill="x", expand=True, padx=10)
        ...
        sc = ttk.Combobox(sl, textvariable=self.status_var, values=STATUS_OPTIONS,
                          state="readonly", font=FONT_INPUT, width=14)
        sc.pack(anchor="w", pady=2)
        tk.Frame(body, bg=C["BORDER"], height=1).pack(fill="x", pady=8)
        ...
        self.note = tk.Text(wrap, font=FONT_INPUT, relief="flat", bg=C["PANEL"],
                            fg=C["TEXT"], insertbackground=C["TEXT"],
                            wrap="word", height=5, padx=6, pady=6)
        self.note.pack(fill="both", expand=True, padx=1, pady=1)
        if self.existing:
            self.note.insert("1.0", self.existing.get("notiz", ""))
    ...
    def _sel_stw(self, s):
        ...
        for k, b in self._stw_btns.items():
            b.configure(bg=C["PRIMARY"] if k == s else C["BORDER"],
                        fg="white" if k == s else C["TEXT"])
    ...
    def _confirm(self):
        ...
        if not notiz:
            messagebox.showwarning("Fehler", "Bitte Notiz eingeben.", parent=self)
            return
        data = {
            "notiz": notiz,
        ...
        if not self.tf_mode:
            ...
            if not zuege:
                messagebox.showwarning("Fehler", "Bitte Zugnummer eingeben.", parent=self)
                return
        ...
        if self.existing:
            data["id"] = self.existing.get("id", 0)
...
class BaseTab(tk.Frame):
    ...
    def _build_toolbar(self):
        ...
        if not self.tf_mode:
            ...
            for s in ["Alle"] + STW_OPTIONS:
                b = tk.Button(tb, text=s, font=("Segoe UI", 9, "bold"),
                              bg=C["BORDER"], fg=C["TEXT"], relief="flat", cursor="hand2",
                              padx=10, pady=4, bd=0, command=lambda s=s: self._apply_filter(s))
                b.pack(side="left", padx=3)
                self._fbtns[s] = b
        ...
        self._sv.trace("w", lambda *a: self._refresh())
        se = tk.Entry(tb, textvariable=self._sv, font=FONT_INPUT, relief="flat",
                      bg=C["PANEL"], fg=C["TEXT"], insertbackground=C["TEXT"],
                      highlightthickness=1, highlightbackground=C["BORDER"],
                      highlightcolor=C["PRIMARY"], width=20)
    ...
    def _build_table(self):
        ...
        # Kopfzeile
        cols = ["#", "STW", "Gleis", "Fahrt", "Züge", "Notiz", "Prio", "Status", "Erstellt", ""]
        if self.tf_mode:
            cols = ["#", "Notiz", "Prio", "Status", "Erstellt", ""]
        ...
        for col in cols:
            ...
            lbl.pack(side="left", fill="x" if expand else "none", expand=expand)

        sc_wrap = tk.Frame(outer, bg=C["BG"]); sc_wrap.pack(fill="both", expand=True)
        self.canvas = tk.Canvas(sc_wrap, bg=C["BG"], highlightthickness=0)
        sb = tk.Scrollbar(sc_wrap, orient="vertical", command=self.canvas.yview)
    ...
    def _refresh(self):
        ...
        for i, note in enumerate(display):
            ...
            status = note.get("status", "Offen")
            prio   = note.get("prio", "Mittel")
            ...
            tk.Label(row, text=note.get("notiz",""), font=FONT_TABLE,
                     bg=rbg, fg=C["TEXT"], anchor="w", wraplength=400,
                     justify="left").pack(side="left", fill="x", expand=True, padx=6, pady=8)

            # Prio Badge
            tk.Label(row, text=prio, font=("Segoe UI", 8, "bold"),
                     bg=PRIO_COLORS.get(prio, "#888"), fg="white",
                     padx=6, pady=2).pack(side="left", padx=4, pady=8)

            # Status Badge
            tk.Label(row, text=status, font=("Segoe UI", 8, "bold"),
                     bg=STATUS_COLORS.get(status, "#888"), fg="white",
                     padx=6, pady=2).pack(side="left", padx=4, pady=8)
            ...
            # Edit
            e = tk.Label(row, text=" ✎ ", font=("Segoe UI", 11),
    ...
    def _add(self, data):
        ...
        save_json(self.data_path, self.notes)
        self._refresh()
    ...
    def _delete(self, idx, note):
        if messagebox.askyesno("Löschen?",
            f"Notiz wirklich löschen?\n\n\"{note.get('notiz','')[:60]}\"", parent=self):
            self.notes.pop(idx)
            save_json(self.data_path, self.notes)
            self._refresh()

    def _export_csv(self):
        path = os.path.join(os.path.dirname(self.data_path),
                            f"export_{datetime.date.today().isoformat()}.csv")
    ...
    def _print(self):
        ...
        try:
            ...
            messagebox.showinfo("Drucken", f"Druckauftrag fehlgeschlagen:\n{ex}", parent=self)


# ── STW-Tab ────────────────────────────────────────────────────────────────────
class STWTab(BaseTab):
    def __init__(self, parent):
        super().__init__(parent, DATA_FILE, tf_mode=False)


# ── TF-Tab ────────────────────────────────────────────────────────────────────
class TFTab(BaseTab):
    def __init__(self, parent):
        super().__init__(parent, TF_FILE, tf_mode=True)

    def _build_toolbar(self):
        tb = tk.Frame(self, bg=C["BG"]); tb.pack(fill="x", padx=16, pady=10)
        make_btn(tb, "+  TF-Notiz hinzufügen", self._open_add, pady=7).pack(side="left")
        tk.Label(tb, text="Suche:", font=FONT_LABEL, bg=C["BG"], fg=C["SUBTEXT"]).pack(side="left", padx=10)
    ...
    def _do_update(win, remote):
        try:
            ...
            with urllib.request.urlopen(DOWNLOAD_URL, timeout=15) as r:
                new_code = r.read()
            shutil.copy2(script, script + f".v{APP_VERSION}.bak")
            with open(script, "wb") as f:
                f.write(new_code)
            win.destroy()
            messagebox.showinfo("Update", f"v{remote} installiert!\nBitte Programm neu starten.", parent=parent)
        except Exception as ex:
...
class NotizenApp(tk.Tk):
    ...
    def _build_header(self):
        self._hdr = tk.Frame(self, bg=C["PRIMARY"])
        ...
        self._top_btn = tk.Button(self._hdr, text="📌 Vorne", font=("Segoe UI", 9),
                                   bg=C["PRIMARY_H"], fg="white", relief="flat",
        ...
        # Versionslabel unten rechts → Status-Bar
        if not hasattr(self, "_statusbar_built"):
            self._statusbar_built = True
...
if __name__ == "__main__":
    app = NotizenApp()
    # Status-Bar mit Version unten rechts
    sb = tk.Frame(app, bg=C["BORDER"], height=22)
    sb.pack(fill="x", side="bottom")
    tk.Label(sb, text=f"v{APP_VERSION}", font=("Segoe UI", 8),
             bg=C["BORDER"], fg=C["SUBTEXT"]).pack(side="right", padx=10)
    # Beim Start leise auf Update prüfen
